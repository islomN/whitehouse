        ...
        <md-card-content>
            ...
            <div class=" mt-5 ">
                <table class="table">
                    ...
                    <tbody>
                    <tr v-for="(item, i) in info.planAchievements">
                        <td>{{ i + 1}}</td>
    ...
    export default {
        ...
        methods: {
            save(){
                ...
                this.$api.post('/api/PlanAchievement/AddPlanAchievement', this.model).then(
                    response => {
                        this.$v.$reset();
                        this.info.planAchievements.push(response.data.result)
            ...
            deleteItem(index){
                ...
                let factExpense = this.info.planAchievements[index]
                if(!factExpense){
                    return;
                }
                this.$api.delete('/api/PlanAchievement/RemovePlanAchievement/'+factExpense.id).then(
                    response => {
                        this.successNotifyMini("Успешно удалено!!!")
                        this.info.planAchievements.splice(index, 1);
                    },
                    error => {
                        this.errorNotify(error.response.data.error.errorMessage)
                        console.log(error);
